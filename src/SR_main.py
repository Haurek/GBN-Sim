...
def receive_thread(connectSocket, fp):
    receiver = SRReceiver.SRReceiver(connectSocket, fp)
    receiver.run()


def send_thread(connectSocket, addr, fp):
    sender = SRSender.SRSender(connectSocket, addr, fp)
    sender.run()
...
# file send to port 12000
receiver_fp1 = open("./receive/SRReceive1.jpg", 'wb')
...
receiver_fp2 = open("./receive/SRReceive2.jpg", 'wb')
